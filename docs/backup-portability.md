...
| Android 7.0 / 13 / 16 (API 24, 33, 36) google_apis x86_64 emulators (CI `integration-android`, ubuntu-24.04) | platform sqflite | Same Dart suite via `tool/integration_android.sh <api>` on a headless accelerated emulator at each level: API 24 is the supported minimum, API 33 the established baseline, API 36 the current compile/target level. Restore through the real system file picker is additionally driven end-to-end on an API 33 emulator (fixture file in Downloads, DocumentsUI selection, validated preview, confirmed replacement). |
